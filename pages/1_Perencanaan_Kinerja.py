...
try:
    ...
    for _, row in df.iterrows():
        ...
        if teks_upper == "PERENCANAAN KINERJA":
            current_component = "PERENCANAAN KINERJA"
            current_sub = None
            continue
        ...
        if current_component != "PERENCANAAN KINERJA":
            continue

        # Ambil sub komponen 1.a, 1.b, 1.c
        if no_value.lower() in ["1.a", "1.b", "1.c"]:
        ...
        # Ambil baris inovasi sebagai bagian tersendiri
    ...
    if sub_komponen.empty:
        st.error(
            "Data sub komponen Perencanaan tidak ditemukan. "
            "Cek baris 1.a, 1.b, 1.c di Google Sheet."
        )
        st.stop()
    ...
    st.error(f"Gagal membaca data Perencanaan dari Google Sheet: {e}")
    st.stop()
...
render_component_page(
    ...
    sidebar_title="Perencanaan Kinerja",
    sidebar_page_path="pages/1_Perencanaan_Kinerja.py",
    sidebar_icon="📘",
    inovasi=inovasi
)
