...
try:
    ...
    for _, row in df.iterrows():
        ...
        if teks_upper == "PELAPORAN KINERJA":
            current_component = "PELAPORAN KINERJA"
            current_sub = None
            continue
        ...
        if current_component != "PELAPORAN KINERJA":
            continue

        if no_value.lower() in ["3.a", "3.b", "3.c"]:
    ...
    if sub_komponen.empty:
        st.error(
            "Data sub komponen Pelaporan tidak ditemukan. "
            "Cek baris 3.a, 3.b, 3.c di Google Sheet."
        )
        st.stop()
    ...
    st.error(f"Gagal membaca data Pelaporan dari Google Sheet: {e}")
    st.stop()
...
render_component_page(
    ...
    sidebar_title="Pelaporan Kinerja",
    sidebar_page_path="pages/3_Pelaporan_Kinerja.py",
    sidebar_icon="📙",
    inovasi=inovasi
)