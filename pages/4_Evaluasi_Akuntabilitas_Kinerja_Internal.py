...
st.set_page_config(
    page_title="Evaluasi Akuntabilitas Kinerja Internal",
    page_icon="📕",
    layout="wide"
)
...
try:
    ...
    for _, row in df.iterrows():
        ...
        if teks_upper == "EVALUASI AKUNTABILITAS KINERJA INTERNAL":
            current_component = "EVALUASI AKUNTABILITAS KINERJA INTERNAL"
            current_sub = None
            continue
        ...
        if current_component != "EVALUASI AKUNTABILITAS KINERJA INTERNAL":
            continue

        if no_value.lower() in ["4.a", "4.b", "4.c"]:
    ...
    if sub_komponen.empty:
        st.error(
            "Data sub komponen Evaluasi Akuntabilitas Kinerja Internal tidak ditemukan. "
            "Cek baris 4.a, 4.b, 4.c di Google Sheet."
        )
        st.stop()
    ...
    st.error(f"Gagal membaca data Evaluasi Akuntabilitas Kinerja Internal dari Google Sheet: {e}")
    st.stop()
...
render_component_page(
    ...
    sidebar_title="Evaluasi Akuntabilitas Kinerja Internal",
    sidebar_page_path="pages/4_Evaluasi_Akuntabilitas_Kinerja_Internal.py",
    sidebar_icon="📕",
    inovasi=inovasi
)